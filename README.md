...
一个部署到 GitHub Pages 的静态文献库。GitHub Actions 在每周二和周五北京时间 09:00（UTC 01:00）查询 Europe PMC，收录近八年的类器官、器官芯片相关论文。原始研究、综述和预印本分别展示，新增条目默认「待审核」。首页提供最新发表、重点期刊、病原体、药物筛选和 AI 赋能入口，并支持近 30 天、发表日期排序、器官系统、常见期刊与开放获取筛选。
...
打开相应年份的 `docs/data/articles/YYYY.json` 找到条目的 `key`（优先使用 `doi:...`，无 DOI 时使用 `pmid:...`）。在 `reviews.json` 增加覆盖项，例如：
...
文献按 Europe PMC 的 `firstPublicationDate` 排序，并分别保存 `electronicPublicationDate`。来源提供时展示作者单位、开放全文链接和许可信息；通讯作者没有可靠标识时不猜测。原文 DOI 链接和可用的开放全文链接在卡片展示。采集以 Europe PMC 官方 API 为主，其记录包含 PubMed PMID；当前未对全部记录额外调用 Crossref 或 OpenAlex，缺失字段仍如实留空。
